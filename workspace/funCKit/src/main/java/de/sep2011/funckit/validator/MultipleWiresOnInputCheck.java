...
/**
 * Checks if an {@link Input} in the {@link Circuit} has more than one
 * {@link Wire} connected.
 */
public class MultipleWiresOnInputCheck implements Check {
    ...
    /**
     * {@inheritDoc} In this case check if an {@link Input} in the
     * {@link Circuit} has more than one {@link Wire} connected. Result will
     * contain all affected Wires and the affected Brick.
     */
    @Override
    ...
    private boolean checkCircuit(Circuit circuit) {
        ...
        // look for inputs with more than one wire
        for (Element e : circuit.getElements()) {
            if (e instanceof Brick) {
                Brick b = (Brick) e;
                for (Input i : b.getInputs()) {
                    if (i.getWires().size() > 1) {

                        // mark all wires as flaws
                        flawElements.addAll(i.getWires());

                        // mark the brick as flaw
                        flawElements.add(b);
                        hasFlaw = true;
                    }
                }
