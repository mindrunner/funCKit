...
/**
 * Listener object for actions that indicate a select all event.
 */
public class SelectAllActionListener extends AbstractAction {
    /**
     * 
     */
    private static final long serialVersionUID = -2305272226419697352L;
     ...
     */
    public SelectAllActionListener(View view, Controller controller) {
        this.controller = controller;
    }

    /**
     * Trigger to select all elements in current circuit.
     *
     * @param event ActionEvent information when selection is initiated.
     */
    @Override
    public void actionPerformed(ActionEvent event) {
        ...
        if (editPanelModel == null) {
            Log.gl().debug("Can't select all, no EditPanel selected");
            return;
        }

        editPanelModel.setSelectedElements(new LinkedHashSet<Element>(
                editPanelModel.getCircuit().getElements()));
    }
}