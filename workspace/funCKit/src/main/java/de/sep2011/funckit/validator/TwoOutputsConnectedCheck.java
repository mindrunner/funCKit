...
/**
 * Checks if wires connect two {@link Output}s in a circuit object.
 */
public class TwoOutputsConnectedCheck implements Check {
    ...
    /**
     * Performs concrete check on given circuit object. If certain wires connect
     * two outputs, the affected wire and the connected bricks are returned in
     * <code>flawElements</code> of {@link Result}. Result returns false if one
     * or several wires connect two outputs.
     */
    @Override
    ...
    private boolean checkCircuit(Circuit circuit) {
         ...
         * Outputs and if so, add them to result set.
         */
        for (Element e : circuit.getElements()) {
            if (e instanceof Wire) {
                ...
                if (a1 instanceof Output && a2 instanceof Output) {
