...
/**
 * Listener which listens to a LAF Change request from the View.
 */
public class LafChangeActionListener implements ActionListener {
     ...
     */
    public LafChangeActionListener(View view, Controller controller) {
        this.view = view;
        this.controller = controller;
    }

    /**
     * Action triggered when look and feel gets changed.
     * 
     * @param event
     *            Event containing LookAndFeel name.
     */
    @Override
    public void actionPerformed(ActionEvent event) {
        Settings settings = controller.getSessionModel().getSettings();
        boolean oldAndNewTheSame = settings.getString(Settings.LOOK_AND_FEEL).equals(
                event.getActionCommand());

        settings.set(Settings.LOOK_AND_FEEL, event.getActionCommand());
