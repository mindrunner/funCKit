...
/**
 * Creates a Identity consisting of two {@link Not}s.
 */
public class IdentityFactory extends AbstractCircuitFactory {

    private final IdPoint point;

    /**
     * Creates a new {@link IdentityFactory}.
     */
    public IdentityFactory() {
        circuit = new CircuitImpl();

        point = new IdPoint(new Point());
        point.setName("ID-POINT");
        circuit.addBrick(point);

    }

    @Override
    public ComponentType getComponentTypeForCircuit() {
        ComponentType type = new ComponentTypeImpl(circuit, Language.tr("circuitFactory.identity"));
        type.setHeight(40);
        type.setWidth(40);
        type.getInputs().add(point.getInputA());
        type.getOutputs().add(point.getOutputO());
        type.setName(point.getInputA(), "a");
        type.setName(point.getOutputO(), "o");
